---
status: asserted
likelihood: 1.0
sources:
    - ../sources.kb/abby.md
depends:
    - ../claims.kb/kids-need-early-win.md
    - ../claims.kb/session-zero-improves-buy-in.md
tags: [session-zero, motivation]
---

Abby agreed to try Python/Pygame Zero for a session or two, but
"reluctantly" — this is compliance, not intrinsic buy-in. That makes
`kids-need-early-win.md` load-bearing in a way it wasn't for a kid who
switched tools eagerly: if the very first Python session doesn't produce
something satisfying fast, reluctant consent has nothing to convert it
into real engagement, and the honest outcome is "back to Scratch," not
"push through." The trial needs a concrete, named checkpoint (not a vague
"we'll see") so reluctance doesn't quietly become the default outcome
without an actual conversation.
